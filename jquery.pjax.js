...
// Returns whatever $.ajax returns.
    function pjax(options) {
        ...
        // SLING UPDATE
        // Sling selectors should handle this; Commenting this out removes the _pjax query param
        if(false) {
        ...
        if (xhr.readyState > 0) {
            if (options.push && !options.replace) {
                ...
                // SLING UPDATE
                /***** Updated stripPjaxParam to remove selectors from (options.url) *********/
    ...
    // SLING UPDATE
    // New method to add Sling Selectors to options.url
    function addSelector(options, selector) {
        var i = options.url.indexOf('.');
        var start = options.url.substring(0, i);
        var end = options.url.substring(i, options.url.length);
